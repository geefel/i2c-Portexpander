...
void writeToMCP23N17(uint8_t expanAdr, uint8_t reg, uint8_t pinset, uint8_t regDataIndex);
uint8_t readFromMCP23N17(uint8_t expanAdr, uint8_t reg, uint8_t regDataIndex);
...
void setRegData(uint8_t expanAdr, uint8_t reg, uint8_t dat) {
    regData[expanAdr][reg] = dat;
}

uint8_t getRegData(uint8_t expanAdr, uint8_t reg) {
    return regData[expanAdr][reg];
}

void writeToMCP23N17(uint8_t expanAdr, uint8_t reg, uint8_t pinSet, uint8_t regDataIndex) {
    setRegData(expanAdr, regDataIndex, pinSet);
    
    startI2c();
    sendI2cData(0b01000000 + (expanAdr << 1) + 0);   //rw = 0: schreiben zum Client)
    readACK();
    sendI2cData(reg);
    readACK();
    sendI2cData(pinSet);
...
uint8_t readFromMCP23N17(uint8_t expanAdr, uint8_t reg, uint8_t regDataIndex) {
    ...
    readACK();
    startI2c();
    sendI2cData(0b01000000 + (expanAdr << 1) + 1);   //rw = 1: lesen vom Client)
    readACK();
    erg = getI2cData();
    sendACK();
    stopI2c();    
    
    setRegData(expanAdr, regDataIndex, erg);
    return erg;
}
...
void set_I_O_DIRECTION_REGISTER(uint8_t expanAdr, uint8_t gpio_ab, uint8_t pinSet) {
    ...
    if (gpio_ab == 0) { 
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? IODIRA_IOCON_BANK_0 : IODIRA_IOCON_BANK_1;
        regDataIndex = IODIRA;
    }
    else {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? IODIRB_IOCON_BANK_0 : IODIRB_IOCON_BANK_1;
        regDataIndex = IODIRB;
    }
    writeToMCP23N17(expanAdr, reg, pinSet, regDataIndex);
}
...
uint8_t get_I_O_DIRECTION_REGISTER(uint8_t expanAdr, uint8_t gpio_ab) {
    ...
    return readFromMCP23N17(expanAdr, reg, regDataIndex);
}
...
void set_INPUT_POLARITY_REGISTER(uint8_t expanAdr, uint8_t gpio_ab, uint8_t pinSet) {
    ...
    uint8_t regDataIndex;
    if (gpio_ab == 0) {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? IPOLA_IOCON_BANK_0 : IPOLA_IOCON_BANK_1;
        regDataIndex = IPOLA;
    }
    else {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? IPOLB_IOCON_BANK_0 : IPOLB_IOCON_BANK_1;
        regDataIndex = IPOLB;
    }
    writeToMCP23N17(expanAdr, reg, pinSet, regDataIndex);
}
...
void set_INTERRUPT_ON_CHANGE_PINS(uint8_t expanAdr, uint8_t gpio_ab, uint8_t pinSet) {
    ...
    uint8_t regDataIndex;
    if (gpio_ab == 0) {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? GPINTENA_IOCON_BANK_0 : GPINTENA_IOCON_BANK_1;
        regDataIndex = GPINTENA;
    }
    else {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? GPINTENB_IOCON_BANK_0 : GPINTENB_IOCON_BANK_1; 
...
void set_INTERRUPPT_DEFAULT_VALUE_REGISTER(uint8_t expanAdr, uint8_t gpio_ab, uint8_t pinSet) {
    ...
    uint8_t regDataIndex;
    if (gpio_ab == 0) {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? DEFVALA_IOCON_BANK_0 : DEFVALA_IOCON_BANK_1;
        regDataIndex = DEFVALA;
    }
    else {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? DEFVALB_IOCON_BANK_0 : DEFVALB_IOCON_BANK_1;
...
void set_INTERRUPT_ON_CHANGE_CONTROL_REGISTER(uint8_t expanAdr, uint8_t gpio_ab, uint8_t pinSet) {
    ...
    uint8_t regDataIndex;
    if (gpio_ab == 0) {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? INTCONA_IOCON_BANK_0 : INTCONA_IOCON_BANK_1;
        regDataIndex = INTCONA;
    }
    else {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? INTCONB_IOCON_BANK_0 : INTCONB_IOCON_BANK_1;
...
void set_I_O_EXPANDER_CONFIGURATION_REGISTER(uint8_t expanAdr, uint8_t gpio_ab, uint8_t configBits) {
    ...
    uint8_t regDataIndex;
    if (gpio_ab == 0) {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? IOCON_IOCON_BANK_0 : IOCON_IOCON_BANK_1;
        regDataIndex = IOCON;
    }
    else {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? IOCON_IOCON_BANK_0 : IOCON_IOCON_BANK_1;
    ...
    writeToMCP23N17(expanAdr, reg, configBits, regDataIndex);
}
...
void set_GPIO_PULL_UP_RESISTOR_REGISTER(uint8_t expanAdr, uint8_t gpio_ab, uint8_t pinSet) {
    ...
    uint8_t regDataIndex;
    if (gpio_ab == 0) {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? GPPUA_IOCON_BANK_0 : GPPUA_IOCON_BANK_1;
        regDataIndex = GPPUA;
    }
    else {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? GPPUB_IOCON_BANK_0 : GPPUB_IOCON_BANK_1;
        regDataIndex = GPPUB;
    }
    writeToMCP23N17(expanAdr, reg, pinSet, regDataIndex);
}
...
uint8_t get_INTERRUPT_FLAG_REGISTER(uint8_t expanAdr, uint8_t gpio_ab) {
    ...
    uint8_t regDataIndex;
    if (gpio_ab == 0) {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? INTFA_IOCON_BANK_0 : INTFA_IOCON_BANK_1;
        regDataIndex = INTFA;
    }
    else {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? INTFB_IOCON_BANK_0 : INTFB_IOCON_BANK_1;
        regDataIndex = INTFB;
    }
    return readFromMCP23N17(expanAdr, reg, regDataIndex);
}
...
uint8_t get_INTERRUPT_CAPTURED_VALUE(uint8_t expanAdr, uint8_t gpio_ab) {
    ...
    uint8_t regDataIndex;
    if (gpio_ab == 0) {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? INTCAPA_IOCON_BANK_0 : INTCAPA_IOCON_BANK_1;
        regDataIndex = INTCAPA;
    }
    else {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? INTCAPB_IOCON_BANK_0 : INTCAPB_IOCON_BANK_1;
...
void set_GENERAL_PURPOSE_I_O_PORT_REGISTER(uint8_t expanAdr, uint8_t gpio_ab, uint8_t pinSet) {
    ...
    uint8_t regDataIndex;
    if (gpio_ab == 0) {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? GPIOA_IOCON_BANK_0 : GPIOA_IOCON_BANK_1;
        regDataIndex = GPIOA;
    }
    else {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? GPIOB_IOCON_BANK_0 : GPIOB_IOCON_BANK_1;
        regDataIndex = GPIOB;
    }
    writeToMCP23N17(expanAdr, reg, pinSet, regDataIndex);
}
...
void set_OUTPUT_LATCH_REGISTER_0(uint8_t expanAdr, uint8_t gpio_ab, uint8_t pinSet) {
    ...
    uint8_t regDataIndex;
    if (gpio_ab == 0) {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? OLATA_IOCON_BANK_0 : OLATA_IOCON_BANK_1;
        regDataIndex = OLATA;
    }
    else {
        reg = (regData[expanAdr][IOCON] & (1 << BANK)) == 0 ? OLATB_IOCON_BANK_0 : OLATB_IOCON_BANK_1;
        regDataIndex = OLATB;
    }
    writeToMCP23N17(expanAdr, reg, pinSet, regDataIndex);
}
